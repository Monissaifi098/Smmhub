    ...
    return {
        reply_markup: {
            inline_keyboard: [
                ...
                [{ text: '✅ Join Kar Liya — Verify Karo!', callback_data: 'verify_join' }]
            ]
        }
    };
}
...
async function sendJoinGate(chatId) {
    // Step 1: Typing animation for suspense
    await bot.sendChatAction(chatId, 'typing');
    await sleep(700);

    // Step 2: Eye-catching intro message
    ...
    // Step 3: Self react with fire emoji on intro
    try {
        await bot.setMessageReaction(chatId, introMsg.message_id, [{ type: 'emoji', emoji: '🔥' }]);
    } catch (e) {}

    await sleep(500);

    // Step 4: Animated loading dots
    const loadingMsg = await bot.sendMessage(chatId, `🔍 Checking access`);
    for (const frame of ['🔍 Checking access .', '🔍 Checking access ..', '🔍 Checking access ...', '🔒 Access LOCKED ❌']) {
        await sleep(350);
        try {
            await bot.editMessageText(frame, { chat_id: chatId, message_id: loadingMsg.message_id });
        } catch (e) {}
    }

    // Step 5: Self react with lock emoji
    try {
        await bot.setMessageReaction(chatId, loadingMsg.message_id, [{ type: 'emoji', emoji: '🤔' }]);
    } catch (e) {}
    ...
    // Step 6: Premium unlock message
    const gateMsg = await bot.sendMessage(
        chatId,
        `🚀 *SMM HUB — Exclusive Access Panel* 🚀\n\n` +
        `━━━━━━━━━━━━━━━━━━━━━\n` +
        `🔐 *Bot sirf members ke liye hai!*\n` +
        `━━━━━━━━━━━━━━━━━━━━━\n\n` +
        `💎 Ye bot join karne ke baad milega:\n` +
        `   🎯 Free Instagram Views\n` +
        `   💰 Daily Cash Rewards\n` +
        `   🤝 Refer & Earn (10K Views!)\n` +
        `   🛍️ SMM Services at Best Price\n\n` +
        `━━━━━━━━━━━━━━━━━━━━━\n` +
        `👇 *Dono channels join karo — FREE hai!*\n` +
        `━━━━━━━━━━━━━━━━━━━━━`,
        { parse_mode: 'Markdown', ...joinChannelsKeyboard() }
    );

    // Step 7: Self react with crown/gem on gate message
    try {
        await bot.setMessageReaction(chatId, gateMsg.message_id, [{ type: 'emoji', emoji: '👑' }]);
    } catch (e) {}

    await sleep(800);

    // Step 8: Final push notification
    const pushMsg = await bot.sendMessage(
        chatId,
        `⏰ *Jaldi karo bhai!*\n\n` +
        `Join karo aur fauran bot use karo 🚀🔥\n` +
        `Ek click — lifetime access! 🎁`,
        { parse_mode: 'Markdown' }
    );

    // Step 9: React with fire on final push
    try {
        await bot.setMessageReaction(chatId, pushMsg.message_id, [{ type: 'emoji', emoji: '🎉' }]);
    } catch (e) {}
}
...
async function proceedAfterJoin(chatId, from, payload) {
    ...
    if (!existing) {
        ...
        // Premium new user welcome animation
        await bot.sendChatAction(chatId, 'typing');
        await sleep(500);

        // Account creation animation
        const createMsg = await bot.sendMessage(chatId, `⚙️ Account bana raha hun`);
        for (const frame of [
            '⚙️ Account bana raha hun .',
            '⚙️ Account bana raha hun ..',
            '⚙️ Account bana raha hun ...',
            '✅ Account Ready!'
        ]) {
            await sleep(350);
            try {
                await bot.editMessageText(frame, { chat_id: chatId, message_id: createMsg.message_id });
            } catch (e) {}
        }

        try {
            await bot.setMessageReaction(chatId, createMsg.message_id, [{ type: 'emoji', emoji: '🎉' }]);
        } catch (e) {}
        ...
        const welcomeMsg = await bot.sendMessage(
            ...
            `🔥 *SMM Hub me aapka swagat hai!* 🔥\n\n` +
            `━━━━━━━━━━━━━━━━━━━━━\n` +
            `🎁 *Aapka FREE Gift Ready Hai:*\n` +
            `   🎯 10,000 Views milenge referral pe\n` +
            `   💰 Daily ₹2 reward — free mein!\n` +
            `   🛍️ India ka best SMM panel\n` +
            `━━━━━━━━━━━━━━━━━━━━━\n\n` +
            `👇 Menu se kuch bhi select karo, shuru ho jao! 💪`,
            { parse_mode: 'Markdown' }
        );

        try {
            await bot.setMessageReaction(chatId, welcomeMsg.message_id, [{ type: 'emoji', emoji: '🔥' }]);
        } catch (e) {}
        ...
        // Premium returning user welcome
        await bot.sendChatAction(chatId, 'typing');
        await sleep(500);

        const returnEmojis = ['🔥', '💪', '⚡', '🚀', '💎', '👑'];
        const randomEmoji = returnEmojis[Math.floor(Math.random() * returnEmojis.length)];

        const returnMsg = await bot.sendMessage(
            chatId,
            `${randomEmoji} *AAGAYE BOSS — ${username}!* ${randomEmoji}\n\n` +
            `🎯 Wapas aa gaye ho!\n` +
            `💰 Dekho kya naya hai aaj!\n\n` +
            `👇 Chalo shuru karte hain!`,
            { parse_mode: 'Markdown', ...mainKeyboard() }
        );

        try {
            await bot.setMessageReaction(chatId, returnMsg.message_id, [{ type: 'emoji', emoji: randomEmoji }]);
        } catch (e) {}
        ...
        if (data === 'verify_join') {
            ...
            if (!joined) {
                await bot.answerCallbackQuery(query.id, {
                    text: '❌ Bhai abhi tak join nahi kiye! Dono channels join karo phir verify karo 👇',
                    show_alert: true
                });
                return;
            }
            ...
            // Success popup
            await bot.answerCallbackQuery(query.id, { text: '🎉 Verified! Welcome to SMM Hub 🔥' });

            // Remove the join-gate message
            try { await bot.deleteMessage(chatId, query.message.message_id); } catch (e) {}

            // Premium verification success animation
            await bot.sendChatAction(chatId, 'typing');
            await sleep(400);

            // Animated verification sequence
            const verifyMsg = await bot.sendMessage(chatId, `🔄 Verifying membership`);
            for (const frame of [
                ...
                await sleep(380);
                try {
                    await bot.editMessageText(frame, { chat_id: chatId, message_id: verifyMsg.message_id });
                } catch (e) {}
            }

            // React with checkmark
            try {
                await bot.setMessageReaction(chatId, verifyMsg.message_id, [{ type: 'emoji', emoji: '👍' }]);
            } catch (e) {}

            await sleep(300);

            // Celebration burst message
            const celebMsg = await bot.sendMessage(
                chatId,
                `🎊🎊🎊 *WELCOME TO SMM HUB!* 🎊🎊🎊\n\n` +
                `🔥 Tu ab family ka hissa hai bhai!\n` +
                `🚀 Ab kuch bhi rok nahi sakta!\n\n` +
                `💎 Ab tera access unlock ho gaya:\n` +
                `   ✅ Free Views Claim karo\n` +
                `   ✅ Daily ₹ Rewards lo\n` +
                `   ✅ Friends refer karke kamaao\n` +
                `   ✅ SMM Services order karo\n\n` +
                `⬇️ *Ab seedha kaam pe lag ja!* ⬇️`,
                { parse_mode: 'Markdown' }
            );

            // React with fire on celebration message
            try {
                await bot.setMessageReaction(chatId, celebMsg.message_id, [{ type: 'emoji', emoji: '🔥' }]);
            } catch (e) {}
    ...
    try {
        ...
        if (text === '🤝 Refer and Earn') {
            try {
                ...
                console.error('Refer and Earn error:', innerErr);
                bot.sendMessage(chatId, '⚠️ Referral link generate karne me error aaya. Thodi der me try karo.');
            }
            return;
        }
